...
function EditSubscriptionModal({ subscription }) {
  const {
  ...
  useEffect(() => {
    setSelectedCard({});
    if (clientCardsLoading || loadingEditSubscription) {
    ...
    >
      <div style={{ width: 100 }} className="modal-container">
          ...
          >
            ...
            <tbody>
              ...
              <tr className="task-variant tr">
                <td>Expires</td>
                <td>
                  {selectedCard?.expiry_date || subscription?.expiry_date}
                </td>
              </tr>
              <tr className="task-variant tr">
                <td>Pan Encrypted</td>
                <td>{selectedCard?.pan_encrypted || subscription?.pan}</td>
              </tr>
            </tbody>
          </table>
        </div>
        <div style={{ width: "100%" }}>
          ...
          <input
            defaultValue={subscription?.msisdn}
            ref={msisdnRef}
            type="number"
          />
        </div>
        <div style={{ width: "100%" }}>
          ...
          <input
            defaultValue={subscription?.national_id}
            ref={national_idRef}
            type="text"
          />
        </div>
        <div className="modal-btn-section">
          <button
            className="submit-btn"
            onClick={() => {
              return handleCloseEditSubscriptionModal();
            }}
          >
            Cancel
          </button>
          <button
            className={`submit-btn ${loadingEditSubscription && "loading-btn"}`}
            disabled={loadingEditSubscription}
            onClick={() =>
              editSubscription(
                ...
                selectedCard?.pan_encrypted || subscription?.pan,
                selectedCard?.expiry_date || subscription?.expiry_date,
                national_idRef.current["value"] !== ""
                  ? national_idRef.current["value"]
                  : subscription?.national_id,
                subscription?.linked_account,
                handleCloseEditSubscriptionModal
              )
            }
          >
